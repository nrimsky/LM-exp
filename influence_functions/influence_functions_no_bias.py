...
def get_ekfac_factors_and_train_grads(
    ...
    kfac_input_covs = [
        t.zeros((b.get_weights().shape[1], b.get_weights().shape[1])).to(device)
        for b in mlp_blocks
    ]
    kfac_grad_covs = [
    ...
    for i, (data, target) in enumerate(dataset):
        ...
        for i, block in enumerate(mlp_blocks):
            input_cov = t.einsum("...i,...j->ij", block.get_input(), block.get_input())
            kfac_input_covs[i] += input_cov
        loss.backward()
        for i, block in enumerate(mlp_blocks):
            grad_cov = t.einsum("...ik,...jk->ij", block.get_weights().grad, block.get_weights().grad)
            kfac_grad_covs[i] += grad_cov
            train_grads[i].append(block.get_weights().grad.view(-1))
        tot += 1
...
def get_query_grads(
    ...
    for block in mlp_blocks:
        grads.append(block.get_weights().grad)
    return grads
